...
この P4 closeout は compatibility promise だけを意味し、general execution や lifecycle promise を意味しません。より深い `codex-cli` execution、profile/env の受け渡し、runtime-state、そして current internal-only の `spawn-budget`、budget-aware `spawn-candidate`、spawn batch planning、spawn batch item projection、bounded spawn batch apply convenience seam は今も internal-only 実装であり、公開 lifecycle 機能として解釈すべきではありません。
