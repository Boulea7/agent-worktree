...
現在のリポジトリには、ごく薄い Phase 4 public compatibility baseline があります。`compat smoke codex-cli` は Tier 1 runtime に対する最初の bounded で public な end-to-end compatibility checkpoint であり、`doctor` と `compat probe` は他の Tier 1 runtime を明示的な descriptor-only 境界に留めています。
