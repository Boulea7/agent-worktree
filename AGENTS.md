...
- a minimal internal close-oriented helper chain that may derive capability-aware close-readiness, close-candidates, minimal close-targets, target-based close-requests, and close-requested lifecycle markers from runtime-context without becoming actual close support, public selectors, or lifecycle truth
...
The current internal close-oriented helper chain, when present, must remain derived, non-persistent, non-manifest-backed, and non-public: it may prepare capability-aware close-readiness blockers, composed close-candidates, minimal internal close-targets, target-based close-requests, or close-requested lifecycle markers from runtime-context, but it must not reintroduce selector resolution, runtime-state reads, readiness recomputation, or close-consumer preflight, and it must not become actual close support, public selectors, a mutable lifecycle manager, or a source of lifecycle truth. Any `close_requested` marker in this phase remains a future-facing internal event projection only, not close success truth; `closed` still belongs to `close_recorded`.
