...
Another acceptable thin follow-up is to add an internal `spawn-headless-view` helper above an existing `spawn-headless-record` result, so long as it remains a derived pure composition helper, non-persistent, non-manifest-backed, non-public, and limited to composing the existing single-request `spawn-headless-record` result plus the generic `buildExecutionSessionView(...)` helper rather than selector resolution, runtime-state context derivation, readiness recomputation, manifest-backed persistence, public CLI or selector semantics, or a public session-lifecycle API.
That `spawn-headless-view` milestone should stay read-model-composition-only: it may surface a bounded derived `ExecutionSessionView` for future internal callers, but it must remain internal-only, non-public, non-manifest-backed, not lifecycle truth, and not delegated runtime support. It must not reintroduce selector resolution, runtime-state context derivation, readiness or capability recomputation, manifest seeds, public status surfaces, child-creation truth, or adapter-driven delegated-runtime truth.
Another acceptable thin follow-up is to add an internal `spawn-headless-view-batch` helper above an existing `spawn-headless-record-batch` result, so long as it remains a batch-based derived pure composition helper, non-persistent, non-manifest-backed, non-public, and limited to composing the existing batch `spawn-headless-record` result plus the generic `buildExecutionSessionView(...)` helper rather than selector resolution, runtime-state context derivation, manifest-backed persistence, public CLI or selector semantics, summary-policy contracts, or a public session-lifecycle API.
That `spawn-headless-view-batch` milestone should stay batch-based internal read-model composition metadata only: it may preserve record ordering and fail loudly on duplicate-key or invalid-parent identifiers, but it must remain internal-only, non-public, non-manifest-backed, not lifecycle truth, and not delegated runtime support.
