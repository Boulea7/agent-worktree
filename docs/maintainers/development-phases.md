...
- the current barrel-boundary follow-up for this window further tightens `tests/control-plane/internal.test.ts`, `tests/control-plane/index.test.ts`, `tests/selection/internal.test.ts`, `tests/selection/index.test.ts`, and `tests/verification/internal.test.ts` around close-side, spawn-side, spawn-headless wait/close target-apply helpers, closeout-entry helpers, and the intentionally narrower default control-plane / selection entry points, so maintainers can trace those additions without treating them as public-contract expansion
