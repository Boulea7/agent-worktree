...
- the current barrel-boundary follow-up for this window further tightens `tests/control-plane/internal.test.ts`, `tests/control-plane/index.test.ts`, `tests/selection/internal.test.ts`, and `tests/selection/index.test.ts` around close-side and closeout-entry helpers, so maintainers can trace those additions without treating them as public-contract expansion
