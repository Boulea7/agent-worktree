...
Another acceptable thin follow-up is to move the next handoff slice from the older `spawn-headless-input` bridge focus to a narrower internal `spawn-headless-apply` helper above existing `spawn-apply` metadata plus a minimal headless-execution seed, so long as it remains an adapter-neutral derived composition helper, non-persistent, non-manifest-backed, non-public, and limited to deriving `attempt` from `apply.effects.lineage` while whitelisting only explicit execution fields such as `prompt`, `cwd`, `timeoutMs`, or `abortSignal` rather than child runtime execution, child creation, terminal lifecycle truth, manifest-backed persistence, public CLI or selector semantics, or a public session-lifecycle API.
That `spawn-headless-apply` milestone should stay internal bridge metadata only: it may prepare a future internal headless execution payload for internal callers, but it must remain internal-only, non-public, non-manifest-backed, not lifecycle truth, and not delegated runtime support. It must not reintroduce selector resolution, runtime-state reads, readiness or capability recomputation, branch/worktree planning, prompt planning beyond passthrough, delegated-runtime approval decisions, or adapter-driven spawn success.
Another acceptable thin follow-up is to add an internal `spawn-headless-apply-batch` helper above an explicit ordered list of existing `spawn-headless-apply` inputs, so long as it remains a request-list-based derived async composition helper, non-persistent, non-manifest-backed, non-public, and limited to sequentially composing the existing single-request `spawn-headless-apply` helper rather than child planning, branch/worktree creation, manifest-backed persistence, public CLI or selector semantics, summary-policy contracts, or a public session-lifecycle API.
That `spawn-headless-apply-batch` milestone should stay request-list-based internal composition metadata only: it may preserve input order and fail fast on the first error, but it must remain internal-only, non-public, non-manifest-backed, not lifecycle truth, and not delegated runtime support.
Another acceptable thin follow-up is to add an internal `spawn-headless-execute` helper above an existing `spawn-headless-apply` result, so long as it remains a derived async composition helper, non-persistent, non-manifest-backed, non-public, and limited to invoking an explicitly injected bounded headless executor exactly once with the already-shaped internal payload rather than child planning, branch/worktree creation, manifest-backed persistence, public CLI or selector semantics, mutable lifecycle control, or a public session-lifecycle API.
That `spawn-headless-execute` milestone should stay consume-path-only: it may surface bounded internal execution-consumption metadata for internal callers, but it must remain internal-only, non-public, non-manifest-backed, not lifecycle truth, and not delegated runtime support. It must not reintroduce selector resolution, runtime-state reads, readiness or capability recomputation, manifest seeds, public status surfaces, child-creation truth, or adapter-driven delegated-runtime truth.
Another acceptable thin follow-up is to add an internal `spawn-headless-execute-batch` helper above an explicit ordered list of existing `spawn-headless-apply` items, so long as it remains a request-list-based derived async composition helper, non-persistent, non-manifest-backed, non-public, and limited to sequentially composing the existing single-request `spawn-headless-execute` helper rather than child planning, branch/worktree creation, manifest-backed persistence, public CLI or selector semantics, summary-policy contracts, or a public session-lifecycle API.
That `spawn-headless-execute-batch` milestone should stay request-list-based internal execution-consumption metadata only: it may preserve input order and fail fast on the first error, but it must remain internal-only, non-public, non-manifest-backed, not lifecycle truth, and not delegated runtime support.
Another acceptable thin follow-up is to add an internal `spawn-headless-record` helper above an existing `spawn-headless-execute` result, so long as it remains a derived pure composition helper, non-persistent, non-manifest-backed, non-public, and limited to composing the existing single-request `spawn-headless-execute` result plus the generic `deriveExecutionSessionRecord(...)` helper rather than selector resolution, runtime-state view or context derivation, readiness recomputation, index building, manifest-backed persistence, public CLI or selector semantics, or a public session-lifecycle API.
That `spawn-headless-record` milestone should stay runtime-state-composition-only: it may surface a bounded derived `ExecutionSessionRecord` for future internal callers, but it must remain internal-only, non-public, non-manifest-backed, not lifecycle truth, and not delegated runtime support. It must not reintroduce selector resolution, runtime-state view/context derivation, readiness or capability recomputation, index building, manifest seeds, public status surfaces, child-creation truth, or adapter-driven delegated-runtime truth.
Another acceptable thin follow-up is to add an internal `spawn-headless-record-batch` helper above an explicit ordered list of existing `spawn-headless-execute` items, so long as it remains a request-list-based derived pure composition helper, non-persistent, non-manifest-backed, non-public, and limited to sequentially composing the existing single-request `spawn-headless-record` helper rather than selector resolution, runtime-state view or context derivation, index building, manifest-backed persistence, public CLI or selector semantics, summary-policy contracts, or a public session-lifecycle API.
That `spawn-headless-record-batch` milestone should stay request-list-based internal runtime-state composition metadata only: it may preserve input order and fail fast on the first derivation error, but it must remain internal-only, non-public, non-manifest-backed, not lifecycle truth, and not delegated runtime support.
Another acceptable thin follow-up is to move the active slice to an internal `spawn-headless-context` helper above an existing `spawn-headless-view` result, so long as it remains a bounded derived pure composition helper, non-persistent, non-manifest-backed, non-public, and limited to composing the existing single-request `spawn-headless-view` result plus the generic `deriveExecutionSessionContext(...)` helper with an internal attempt-based selector derived from the existing headless record rather than selector-surface expansion, runtime-state view rebuilding, readiness recomputation, manifest-backed persistence, public CLI or selector semantics, or a public session-lifecycle API.
That `spawn-headless-context` milestone should stay context-composition-only: it may surface a bounded derived `ExecutionSessionContext` for future internal callers, but it must remain internal-only, non-public, non-manifest-backed, not child-creation truth, not runtime truth, not terminal lifecycle truth, and not delegated runtime support. It must not reintroduce selector-surface expansion beyond the generic helper, runtime-state view rebuilding, readiness or capability recomputation, manifest seeds, public status surfaces, or adapter-driven delegated-runtime truth.
Another acceptable thin follow-up is to add an internal `spawn-headless-context-batch` helper above an existing `spawn-headless-view-batch` result, so long as it remains a batch-based derived pure composition helper, non-persistent, non-manifest-backed, non-public, and limited to composing the existing batch `spawn-headless-view` result plus the generic `deriveExecutionSessionContext(...)` helper across internally derived attempt-based selectors while reusing the same shared `ExecutionSessionView` rather than selector-surface expansion, runtime-state view rebuilding, manifest-backed persistence, public CLI or selector semantics, summary-policy contracts, or a public session-lifecycle API.
That `spawn-headless-context-batch` milestone should stay batch-based internal context-composition metadata only: it may preserve input ordering, reuse the same shared view, and fail fast on the first selector or context-derivation error, but it must expose no per-item summary contract and it must remain internal-only, non-public, non-manifest-backed, not child-creation truth, not runtime truth, not terminal lifecycle truth, and not delegated runtime support.
Another acceptable thin follow-up is to move the active slice to an internal `spawn-headless-wait-candidate` helper above an existing `spawn-headless-context` result, so long as it remains a bounded derived pure bridge helper, non-persistent, non-manifest-backed, non-public, and limited to composing the existing single-request `spawn-headless-context` result plus the generic `deriveExecutionSessionWaitReadiness(...)` helper into wait-candidate-shaped metadata rather than calling the generic selector-driven `deriveExecutionSessionWaitCandidate(...)` helper, reopening selector/view/context derivation, widening into `wait-target` / `wait-request` / `wait-consumer` / `wait-consume`, manifest-backed persistence, public CLI or selector semantics, or a public session-lifecycle API.
That `spawn-headless-wait-candidate` milestone should stay wait-bridge-only: it may surface bounded derived `ExecutionSessionWaitCandidate` semantics for future internal callers, but it must remain internal-only, non-public, non-manifest-backed, not child-creation truth, not runtime truth, not terminal lifecycle truth, not polling truth, not timeout scheduling truth, and not actual wait support. It must not reintroduce selector-surface expansion, runtime-state view rebuilding, context derivation, wait-target/request/consumer/consume shaping, manifest seeds, public status surfaces, or lifecycle truth.
Another acceptable thin follow-up is to add an internal `spawn-headless-wait-candidate-batch` helper above an existing `spawn-headless-context-batch` result, so long as it remains a batch-based derived pure bridge helper, non-persistent, non-manifest-backed, non-public, and limited to composing the existing batch `spawn-headless-context` result by reusing the single-request `spawn-headless-wait-candidate` helper rather than calling the generic selector-driven `deriveExecutionSessionWaitCandidate(...)` helper, reopening selector/view/context derivation, widening into `wait-target` / `wait-request` / `wait-consumer` / `wait-consume`, manifest-backed persistence, public CLI or selector semantics, summary-policy contracts, or a public session-lifecycle API.
That `spawn-headless-wait-candidate-batch` milestone should stay batch-based internal wait-bridge metadata only: it may preserve input ordering and fail fast on the first wait-readiness derivation error, but it must expose no per-item summary contract and it must remain internal-only, non-public, non-manifest-backed, not child-creation truth, not runtime truth, not terminal lifecycle truth, not polling truth, not timeout scheduling truth, and not actual wait support.
