...
- Once the repository has a usable Git commit baseline, implementation windows should actively use non-destructive Git commits and branches as archival checkpoints for each completed thin slice or debugging milestone.
- Until the repository has a usable `HEAD`, maintainers must not assume commit-backed checkpoints, commit-derived worktrees, or other `HEAD`-dependent flows are available locally.
