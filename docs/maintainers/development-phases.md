...
- the current maintainer-only Phase 5 slice now satisfies the current internal Phase 5 exit criteria through internal-only verification, selection, promotion, handoff, and handoff-finalization coverage; later work in these areas should be treated as follow-on hardening or Phase 6 prep rather than as evidence that Phase 5 remains open
...
- `verification execution model`, `required-check handling`, and `artifact summaries` are now anchored directly by `tests/verification/execute.test.ts`, `tests/verification/derive.test.ts`, and `tests/verification/artifact-summary.test.ts`, including zero-check incomplete payloads, required-skipped blockers, and execution-derived artifact-summary recommendation semantics
...
- the current closeout-decision single-entry helper is now anchored by `tests/selection/handoff-finalization-closeout-decision-apply.test.ts`, including a canonical verification-to-closeout success path and a sparse-evidence blocker path that keeps zero-check verification payloads from becoming promotable or closeout-ready
- the current selection report-ready and downstream handoff explanation/decision hardening is now anchored by `tests/selection/handoff-report-ready.test.ts`, `tests/selection/handoff-explanation.test.ts`, `tests/selection/handoff-decision.test.ts`, and `tests/selection/handoff-target.test.ts`, which keep accessor-shaped inputs, inherited identity fields, and non-canonical summary reuse from leaking raw runtime errors through the current internal-only handoff chain without widening public review, merge, or lifecycle policy
- the current selection finalization entry and closeout-reader hardening is now anchored by `tests/selection/handoff-finalization-target.test.ts`, `tests/selection/handoff-finalization-request-apply.test.ts`, `tests/selection/handoff-finalization-report-ready.test.ts`, `tests/selection/handoff-finalization-explanation.test.ts`, `tests/selection/handoff-finalization-grouped-projection-summary.test.ts`, `tests/selection/handoff-finalization-grouped-reporting-summary.test.ts`, `tests/selection/handoff-finalization-grouped-reporting-disposition-summary.test.ts`, `tests/selection/handoff-finalization-closure-summary.test.ts`, `tests/selection/handoff-finalization-closeout-summary.test.ts`, `tests/selection/handoff-finalization-closeout-decision.test.ts`, and `tests/selection/handoff-finalization-closeout-decision-apply.test.ts`, which keep readable-object ingress, canonical subgroup reuse, and fail-closed closeout seams aligned without widening public review, merge, or lifecycle policy
- the current promotion front-half and finalization outcome-summary hardening is now anchored by `tests/selection/promotion-result.test.ts`, `tests/selection/promotion-audit.test.ts`, `tests/selection/promotion-report.test.ts`, `tests/selection/handoff-finalization-outcome-summary.test.ts`, and `tests/selection/handoff-finalization-request-summary-shared.test.ts`, which keep own-property-only candidate ingress, readable-object snapshots, canonical request-summary invariants, and raw-error prevention aligned in the current internal-only selection chain
- the current canonical `selectedIdentity` propagation and promotion-reader hardening follow-up is now anchored by `tests/selection/promotion-explanation.test.ts`, `tests/selection/promotion-decision.test.ts`, `tests/selection/promotion-target.test.ts`, and `tests/selection/downstream-identity-guardrails.test.ts`, which keep whitespace-only identity drift, inherited top-level wrappers, and non-canonical blocker arrays from surviving past the current downstream promotion layers without widening public selection payloads
- the current spawn-side canonical hardening is now anchored by `tests/control-plane/runtime-state-spawn-consume.test.ts`, `tests/control-plane/runtime-state-spawn-consume-batch.test.ts`, `tests/control-plane/runtime-state-spawn-candidate.test.ts`, `tests/control-plane/runtime-state-spawn-target.test.ts`, `tests/control-plane/runtime-state-spawn-request.test.ts`, `tests/control-plane/runtime-state-spawn-apply.test.ts`, `tests/control-plane/runtime-state-spawn-apply-batch.test.ts`, `tests/control-plane/runtime-state-spawn-batch-items-apply.test.ts`, `tests/control-plane/runtime-state-spawn-batch-headless-apply.test.ts`, and `tests/control-plane/runtime-state-spawn-headless-apply.test.ts`, which now keep malformed top-level inputs, inherited wrapper fields, sparse batch entries, and non-canonical spawn-request arrays from reaching injected spawn invokers while preserving preflight-first apply composition and without widening public spawn lifecycle support
...
- the current direct wait/close target-apply and apply-batch ingress hardening is now anchored by `tests/control-plane/runtime-state-wait-target-apply.test.ts`, `tests/control-plane/runtime-state-close-target-apply.test.ts`, `tests/control-plane/runtime-state-wait-apply-batch.test.ts`, and `tests/control-plane/runtime-state-close-apply-batch.test.ts`, which keep inherited targets or request containers plus accessor-shaped callbacks from bypassing the current own-property-only request/apply boundary
- the current wait/close request and target ingress hardening is now anchored by `tests/control-plane/runtime-state-wait-request.test.ts`, `tests/control-plane/runtime-state-close-request.test.ts`, `tests/control-plane/runtime-state-wait-target.test.ts`, and `tests/control-plane/runtime-state-close-target.test.ts`, which now keep prototype-backed or accessor-shaped inner identifier fields from surviving into canonical request/target derivation
...
- the current single-entry and nested-snapshot headless follow-up is now anchored by `tests/control-plane/runtime-state-spawn-headless-context.test.ts`, `tests/control-plane/runtime-state-spawn-headless-wait-candidate.test.ts`, `tests/control-plane/runtime-state-spawn-headless-close-candidate.test.ts`, `tests/control-plane/runtime-state-spawn-headless-wait-target.test.ts`, `tests/control-plane/runtime-state-spawn-headless-close-target.test.ts`, `tests/control-plane/runtime-state-spawn-headless-wait-request.test.ts`, `tests/control-plane/runtime-state-spawn-headless-close-request.test.ts`, `tests/control-plane/runtime-state-spawn-headless-wait-request-batch.test.ts`, `tests/control-plane/runtime-state-spawn-headless-close-request-batch.test.ts`, `tests/control-plane/runtime-state-spawn-headless-wait-target-apply.test.ts`, and `tests/control-plane/runtime-state-spawn-headless-close-target-apply.test.ts`, which keep top-level wrapper reads, nested wrapper snapshots, and batch result reuse fail-closed without widening lifecycle or execution contracts
- the current blocked-target headless target-apply preflight is now anchored by `tests/control-plane/runtime-state-spawn-headless-wait-target-apply.test.ts` and `tests/control-plane/runtime-state-spawn-headless-close-target-apply.test.ts`, which keep invalid callbacks or capability resolvers from being silently accepted when every wrapped target is blocked or absent
...
- the current barrel-boundary follow-up for this window further tightens `tests/control-plane/internal.test.ts`, `tests/control-plane/index.test.ts`, `tests/selection/internal.test.ts`, and `tests/selection/index.test.ts` around close-side, spawn-side, spawn-headless wait/close request and target-apply helpers, closeout-entry helpers, repo-internal type-only denylist coverage, and the intentionally narrower default control-plane / selection entry points, alongside the existing verification boundary coverage, so maintainers can trace those additions without treating them as public-contract expansion
...
- the current headless apply/execute/input/record batch ingress hardening is now anchored directly by `tests/control-plane/runtime-state-spawn-headless-input-batch.test.ts`, `tests/control-plane/runtime-state-spawn-headless-apply-batch.test.ts`, `tests/control-plane/runtime-state-spawn-headless-execute-batch.test.ts`, and `tests/control-plane/runtime-state-spawn-headless-record-batch.test.ts`, which keep inherited top-level `items` wrappers and repeated callback reads from bypassing the current headless batch bridge
...
- the current internal coordination-task follow-up now stays internal-only by deriving lightweight delegated-work, blocked-child, verifier-handoff, review-handoff, and closeout-readiness tasks plus board-level ready or dependency-blocked summaries from the existing spawn, headless wait, promotion, handoff, and closeout seams; the current continuation now also includes descendant-coverage truth helpers, shared candidate-context validation, advisory coordination detail/grouping projections, and a ready-queue view above those seams; maintainers should anchor that slice through `tests/control-plane/runtime-state-coordination.test.ts`, `tests/control-plane/runtime-state-descendant-coverage.test.ts`, `tests/control-plane/runtime-state-candidate-context.test.ts`, `tests/control-plane/runtime-state-coordination-details.test.ts`, `tests/control-plane/runtime-state-coordination-groups.test.ts`, `tests/control-plane/runtime-state-coordination-ready-queue.test.ts`, `tests/control-plane/internal.test.ts`, and `tests/control-plane/index.test.ts` without widening public lifecycle, manifest, or execution contracts
- the current Phase 6 headless wait/close prep continues to build on the existing Phase 3/4 control-plane foundation; it should be described as internal continuation on top of that base rather than as a public lifecycle phase jump
...
- before starting any Phase 6 or later development slice, maintainers should also read `docs/maintainers/future-super-agent-harness/execution-and-control-plane.md`, `docs/maintainers/future-super-agent-harness/todo-and-task-tracking.md`, and `docs/maintainers/future-super-agent-harness/review-risks-and-open-questions.md` so execution-boundary, coordination-task, and risk assumptions are part of the same baseline
