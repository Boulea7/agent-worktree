...
- the current internal coordination-task follow-up now stays internal-only by deriving lightweight delegated-work, blocked-child, verifier-handoff, review-handoff, and closeout-readiness tasks plus board-level ready or dependency-blocked summaries from the existing spawn, headless wait, promotion, handoff, and closeout seams; maintainers should anchor that slice through `tests/control-plane/runtime-state-coordination.test.ts`, `tests/control-plane/internal.test.ts`, and `tests/control-plane/index.test.ts` without widening public lifecycle, manifest, or execution contracts
...
- before starting any Phase 6 or later development slice, maintainers should also read `docs/maintainers/future-super-agent-harness/execution-and-control-plane.md` and `docs/maintainers/future-super-agent-harness/review-risks-and-open-questions.md` so execution-boundary and risk assumptions are part of the same baseline
