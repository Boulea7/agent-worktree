...
- a separate internal spawn-headless-view helper may derive a single execution-session view from an existing spawn-headless-record payload by composing the generic execution-session view helper without becoming actual delegated runtime support
- that same spawn-headless-view helper remains read-model internal metadata only: it may return bounded derived `ExecutionSessionView` metadata, but it does not imply child creation, child-runtime truth, terminal lifecycle truth, delegated-runtime truth, or a public session-lifecycle API
- a separate internal spawn-headless-view-batch helper may compose an existing spawn-headless-record-batch payload into a shared execution-session view by composing the generic execution-session view helper without becoming actual delegated runtime support
- that same spawn-headless-view-batch helper remains batch read-model internal metadata only: it preserves record ordering, fails loudly on duplicate-key or invalid-parent identifiers, and does not imply child creation, child-runtime truth, terminal lifecycle truth, delegated-runtime truth, or summary-policy contracts
...
- the same restriction applies to any derived spawn-headless-view metadata: it is internal-only, non-persistent, non-manifest-backed read-model metadata and does not imply actual delegated runtime support, child creation truth, child-runtime execution truth, terminal lifecycle truth, or public lifecycle semantics
- the same restriction applies to any derived spawn-headless-view-batch metadata: it is internal-only, non-persistent, non-manifest-backed batch read-model metadata and does not imply actual delegated runtime support, child creation truth, child-runtime execution truth, terminal lifecycle truth, or summary-policy contracts
...
- public spawn-headless-apply, spawn-headless-execute, spawn-headless-record, or spawn-headless-view CLI surface in `agent-worktree`
