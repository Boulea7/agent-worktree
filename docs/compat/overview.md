...
The merged baseline also contains a deeper internal helper chain for runtime-state, runtime-context, spawn, wait, and close composition, including the current internal-only headless wait/close request projection seams above the existing headless target wrappers, but those helpers remain internal-only and are not a public lifecycle promise.
